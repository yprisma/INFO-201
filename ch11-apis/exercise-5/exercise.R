...
GetReview <- function(movie) {
  ...
  most.recent.review <- flattened[1, ]  
  headline <- most.recent.review$headline
  short.summary <- most.recent.review$summary_short
  link <- most.recent.review$link.url
  
  # Return an list of the three pieces of information from above
  
  return_info <- list(headline = headline, short.summary = short.summary, link = link)
  return(return_info)

}
  
# Test that your function works with a movie of your choice
PiratesInfo <- GetReview("Pirates of the Caribbean")
